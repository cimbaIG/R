...
write.table(results, file = "results.txt", row.names = FALSE,
    col.names = c("Frequency", "Amplitude"))
...
plot(t, y, xlab = "Time t [s]", ylab = "y", type = "l", xlim = c(0, 1),
    ylim = c(-amp, amp))
...
plot(f[1 : (n / 2)],
    mag[1 : (n / 2)], xlab = "Frequency f [Hz]",
        ylab = "Amplitude A", xlim = c(0, 1), ylim = c(0, 1), type = "l")
